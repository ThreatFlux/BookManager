        ...
        if not config_path.exists():
            default_config = {
                ...
                "compilation": {
                    ...
                    "retries": 2,
                    "formats": {
                        "pdf": {"enabled": False, "extra_args": ["--pdf-engine=xelatex"]},  # Disabled by default
                        "docx": {"enabled": True, "extra_args": []},
                        "epub": {"enabled": True, "extra_args": ["--epub-chapter-level=2"]},
                    },
                },
            }
...
class CommandLineParser:
    def __init__(self):
        self.parser = argparse.ArgumentParser(
            description="Book project management tool", formatter_class=argparse.RawDescriptionHelpFormatter
        )
        self._add_arguments()
    ...
    def _add_arguments(self) -> None:
        ...
        self.parser.add_argument("--no-compile", action="store_true", help="Skip manuscript compilation")
        self.parser.add_argument("--report-only", action="store_true", help="Only generate outline report")
        ...
        self.parser.add_argument(
            "--config", type=str, default="config.yaml", help="Path to config file (default: config.yaml)"
        )
        self.parser.add_argument("--force", action="store_true", help="Force reanalysis of all scenes")
        self.parser.add_argument("--quiet", action="store_true", help="Suppress non-error output")
...
class BookManager:
    ...
    def analyze_scenes(self) -> None:
        ...
        total_scenes = sum(len(scenes) for book in self.structure.values() for scenes in book.values())

        with tqdm(total=total_scenes, desc="Analyzing scenes", position=0) as pbar:
            for book_num in self.structure:
                for act_num in self.structure[book_num]:
                    for scene in self.structure[book_num][act_num]:
                        results = analyze_scene(scene["path"], use_cache=not self.args.force)
                        if results:
                            scene.update(results)
                        else:
                            raise BookManagerError(f"Failed to analyze {scene['path']}")
                        pbar.update(1)
    ...
    def generate_outline(self) -> str:
        ...
        for book_num in sorted(self.structure.keys()):
            ...
            for act_num in sorted(self.structure[book_num].keys()):
                ...
                for scene in sorted(self.structure[book_num][act_num], key=lambda x: x["scene_num"]):
                    total_scenes += 1
